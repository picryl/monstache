...
./mongo-shell.sh admin -u "$MONGO_USER_ROOT_NAME" -p "$MONGO_USER_ROOT_PASSWORD" --eval "rs.initiate({ '_id': '$MONGO_REPLICA_SET_NAME', 'version': 1, 'members': $MONGO_REPLICA_SET_MEMBERS });"
